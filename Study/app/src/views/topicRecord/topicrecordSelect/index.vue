...
<ul>
  ...
  <li @click="onError()" :class="$route.path.includes('error')?'active':''">我的错题</li>
...
<!-- 错题 -->
 <div class="buttom" v-show="isShowError">
  <hr>
  <div class="topic" v-show="h.errornumber!=0" v-for="h in topichistoryList" :key="h.answerid" @click="onCategoryError($event, h.cid,h.cateid)">
<p>{{h.categoryname}}练习</p>
<p style="color:red">错{{h.errornumber}}道题</p>
<p>{{h.time}}</p>
</div>
<hr>
...
// console.log(e.currentTarget.innerText.split('练习\n\n')[0]);
// 路由跳转
this.$router.push({name:'recordSelectHistory'},()=>{},()=>{})
    },
      async  onCategoryError(e,cid,cateid){
      console.log(e.currentTarget.innerText.split('\n\n')[2]);
      // 根据时间和token获取对应课程的题目
   await   this.getTopicByTokenTimeData(e.currentTarget.innerText.split('\n\n')[2],cid,cateid)
// console.log(e.currentTarget.innerText.split('练习\n\n')[0]);
// 路由跳转
this.$router.push({name:'recordSelectError'},()=>{},()=>{})
    },
...
// console.log(e.currentTarget.innerText.split('练习\n\n')[0]);
// 路由跳转
this.$router.push({name:'recordSelectCollect'},()=>{},()=>{})
    },
    // 根据时间和token获取对应课程的题目
async getTopicByTokenTimeData(time,cid,cateid){
    let result=await this.$store.dispatch('getTopicByTokenTime',{time:time,token:localStorage.getItem('TOKEN'),cid:cid,cateid:cateid})
    console.log(result);
    if(result=='ok'){
...
this.$router.push({name:'selectHistory'},()=>{},()=>{})
    },
    // 点击我的错题
    onError(){
...
this.isShowCollect=false
this.$router.push({name:'selectError'},()=>{},()=>{})
    },
        // 点击我的收藏
    onCollect(){
this.isShowHistory=false,
...
this.$router.push({name:'selectCollect'},()=>{},()=>{})
