...
// console.log(e.currentTarget.innerText.split('练习\n\n')[0]);
// 路由跳转
this.$router.push({name:'recordFillHistory'},()=>{},()=>{})
    },
...
// console.log(e.currentTarget.innerText.split('练习\n\n')[0]);
// 路由跳转
this.$router.push({name:'recordFillCollect'},()=>{},()=>{})
    },
    // 根据时间和token获取对应填空题课程的题目
async getTopicByTokenTimeData(time,cid,cateid){
    let result=await this.$store.dispatch('getTopicFillByTokenTime',{time:time,token:localStorage.getItem('TOKEN'),cid:cid,cateid:cateid})
    console.log(result);
    if(result=='ok'){
...
this.$router.push({name:'fillHistory'},()=>{},()=>{})
    },
...
this.$router.push({name:'fillCollect'},()=>{},()=>{})

    },
//题目页面，获取填空题目
async getTopicHistoryData(){
    let result=await this.$store.dispatch('getTopicFinishFill',localStorage.getItem('TOKEN'))
    if(result=='ok'){
