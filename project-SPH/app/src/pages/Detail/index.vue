        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 1.加入购物车之前要先通过发送请求的形式通知服务器，让服务器存储加入购物车的数据，方便后续展示购物车的数据并对购物车的数据进行修改 -->
                <!-- 2.点击加入购物车后进行路由跳转，跳转到加入购物车成功页面 -->
...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      ...
      // 当前请求之后的操作要根据请求是成功还是失败来进行对应的操作
      // this.$store.dispatch(' addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum} )
      // 上面的语句本质上是在调用仓库中的addOrUpdateShopCart函数，又因为此函数在仓库中是async,所以她的返回值肯定是一个Promise
      // 而Promise可以根据他的返回值来判断是否成功或失败，成功的话则返回非空字符串，失败返回Promise.reject
      // 但前提是要在仓库中的addOrUpdateShopCart函数中书写返回值return
      // 所以下面语句的result就可以得到成功或失败的语句，在进行相对应的操作
      ...
      try {
        ...
        // 成功之后进行路由跳转
        // 进行路由跳转的时候把已经向服务器请求过了的得到的数据在传给加入购物车成功的页面，但又因为两个页面之间不存在组件的通信
        // 所以可以通过两种方法来传递
        // 1，路由路径参数，但不推荐，因为要传的参数是对象，路径比较复杂
        // 2.利用会话存储，但注意，存储对象时，要进行JSON.stringify,变为字符串，
